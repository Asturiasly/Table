...
namespace ASTImpl {
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                if (type_ == UnaryMinus) return -1 * operand_->Evaluate(sheet);
                else return operand_->Evaluate(sheet);
            }
        ...
        public:
            ...
            void Print(std::ostream& out) const override {
                if (!cell_->IsValid()) {
                    out << FormulaError::Category::Arithmetic;
                }
                else {
                    out << cell_->ToString();
                }
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                ...
                auto cell_interface = sheet.GetCell(*cell_);
                if (cell_interface == nullptr)
                    return res;
                value = cell_interface->GetValue();
                ...
                {
                    bool is_transofrmable_to_double = true;
                    std::string temp_str = std::get<std::string>(value);
                    if (temp_str.size() == 0)
                        return res;

                    if (temp_str[0] != '-' && temp_str[0] != '+' && !(std::isdigit(temp_str[0])))
                    {
                        is_transofrmable_to_double = false;
                    }

                    if (is_transofrmable_to_double)
                    {
                        for (size_t i = 1; i < temp_str.size(); ++i)
                        {
                            if (std::isdigit(temp_str[i]))
                            {
                                continue;
                            }
                            else
                    ...
                    if (is_transofrmable_to_double)
                        res = std::stod(std::get<std::string>(value));
                    else
