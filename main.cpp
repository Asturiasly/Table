...
namespace {
    ...
    void CacheTests()
    {
        ...
        sheet->SetCell("E4"_pos, "=X9");
        sheet->SetCell("X9"_pos, "=M6");
        sheet->SetCell("M6"_pos, "1");

        ASSERT_EQUAL(evaluate("M6"), 1);
        ASSERT_EQUAL(evaluate("X9"), 1);
        ASSERT_EQUAL(evaluate("E4"), 1);
        ASSERT_EQUAL(evaluate("E2"), 1);
    }
...
int main() {
    ...
    RUN_TEST(tr, CacheTests);
    RUN_TEST(tr, SelfRefTest);
}
